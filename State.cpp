...
GameObject* State::getByKey(const string& Key) {
...
void State::addToInv(GameObject* roomObject){
    this->inventory.push_back(roomObject);
}

void State::removeFromInv(GameObject *invObject) {
    inventory.remove(invObject);
}
